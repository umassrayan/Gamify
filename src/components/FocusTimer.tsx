...
interface FocusTimerProps {
    classCode: string;
    setWeeklySeconds: React.Dispatch<React.SetStateAction<number>>;
}
...
const FocusTimer: React.FC<FocusTimerProps> = ({ classCode }) => {
    ...
    return (
        ...
        >
            {/* Left: Timer + Button */}
            <div style={{ textAlign: 'center', flex: 1, gap: '20px' }}>
                <h2 style={{ color: '#594C44' }}>FOCUS TIMER</h2>
                <h1 style={{ fontSize: '5rem', margin: '1rem 0', color: '#594C44' }}>{formatTime(seconds)}</h1>
                <button
                    onClick={toggleTimer}
                    style={{
                        padding: '0.8rem 2rem',
