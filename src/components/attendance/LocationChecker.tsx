...
const LocationChecker: React.FC<LocationCheckerProps> = ({
  ...
  const [targetLocation, setTargetLocation] = useState<LatLngLiteral | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    geocoder.geocode({ address: targetAddress }, (results, status) => {
      if (status === "OK" && results && results[0]) {
        ...
        setGeocodingError(
          `Could not find coordinates for address: ${targetAddress}. Status: ${status}`
        );
  ...
  useEffect(() => {
    ...
    const googleUserLoc = new google.maps.LatLng(
      userLocation.lat,
      userLocation.lng
    );
    const googleTargetLoc = new google.maps.LatLng(
      targetLocation.lat,
      targetLocation.lng
    );
  ...
  return (
    ...
    >
      ...
      >
        ...
        {targetLocation && (
          <Marker position={targetLocation} title="Target Location" />
        )}
