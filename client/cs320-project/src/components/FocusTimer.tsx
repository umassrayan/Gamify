...
const FocusTimer: React.FC<FocusTimerProps> = ({ classCode }) => {
    ...
    const fetchWeeklyFocusTime = async () => {
        ...
        const q = query(collection(db, 'users', currentUser.uid, 'focusLogs'), where('classCode', '==', classCode));
    ...
    return (
        ...
        >
            ...
            <div style={{ textAlign: 'center', flex: 1 }}>
                ...
                <h1 style={{ fontSize: '3rem', margin: '1rem 0', color: '#594C44' }}>{formatTime(seconds)}</h1>
                <button
                    onClick={toggleTimer}
                    style={{
                        padding: '0.8rem 2rem',
            ...
            <div style={{ textAlign: 'right', flex: 1 }}>
                <div style={{ fontSize: '1.2rem', color: '#594C44', fontWeight: 'bold' }}>{formatDuration(weeklySeconds)} Focused this week</div>
            </div>
        </div>
    );
};
